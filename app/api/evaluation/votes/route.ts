...
export async function GET(request: NextRequest) {
  try {
    ...
    const allVotes = await Vote.find({}, { x: 1, y: 1, width: 1, height: 1, comment: 1, likes: 1, _id: 1 }).lean();

    let myVotes: any[] = [];
    let myLikedVoteIds: string[] = [];
    
    if (userId) {
      myVotes = await Vote.find({ userId }, { 
        voteIndex: 1, x: 1, y: 1, width: 1, height: 1, comment: 1, likes: 1, _id: 1 
      }).sort({ voteIndex: 1 }).lean();
      
      // Get all votes this user has liked
      const likes = await VoteLike.find({ oderId: userId }, { voteId: 1, _id: 0 }).lean();
