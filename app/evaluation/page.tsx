...
export default function EvaluationPage() {
  ...
  const [modalComments, setModalComments] = useState<
    { _id: string; comment: string; likes: number; isLiked: boolean }[]
  >([]);
  ...
  const [heatmapData, setHeatmapData] = useState<Uint8ClampedArray | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const imgData = heatCtx.getImageData(
        0,
        0,
        heatCanvas.width,
        heatCanvas.height,
  ...
  const handleHeatmapInteraction = (
    ...
    if (trueX >= 0 && trueX < heatmapWidth) {
      ...
      if (index < heatmapData.length) {
        ...
        if (count > 0) {
          const relevantVotes = allVotes.filter(
            (v) =>
              v.comment &&
              v.x <= trueX &&
              trueX < v.x + v.width &&
            ...
            defaultCoordinates={
              currentVote
                ? {
                    left: currentVote.x,
                    top: currentVote.y,
                    width: currentVote.width,
                    height: currentVote.height,
                  }
                : undefined
            }
            style={{
                ...
                onClick={() => {
                  ...
                  const relevantVotes = allVotes.filter(
                    (v) =>
                      v.comment &&
                      hoverInfo.trueX >= v.x &&
                      hoverInfo.trueX < v.x + v.width &&
                      hoverInfo.trueY >= v.y &&
                      hoverInfo.trueY < v.y + v.height,
                  );
